Got it.
Your issue is **NOT logic-related** — it is a **Markdown formatting problem**.
Some code blocks are **not properly closed**, headings are breaking, and GitHub is rendering everything mixed.

Below is a **FULLY FIXED, CLEAN, COPY-PASTE READY `README.md`** in **EXACTLY the same structure and wording you want**, but **properly formatted** so:

* ✅ Every section renders correctly on GitHub
* ✅ Every code block is selectable & copyable
* ✅ No broken Markdown
* ✅ Same content, just **fixed**
...
(function(){
  ...
  async function _v(){
    const o = document.getElementById('sys_overlay');
    ...
    try{
      ...
      const m = d.licenses[_k];
      const n = new Date().toISOString().split('T')[0];

      if(!m) return _sh(d.title, d.msg_invalid);
      if(m.authorized_target.length && !m.authorized_target.includes(h))
        return _sh("Denied", d.msg_domain_mismatch);
      if(m.expiry && n > m.expiry)
        return _sh(d.title, d.msg_expired);

      o.style.display = 'none';
      if(s) s.style.display = 'block';

      if(m.custom_code.enabled){
        if(s) s.style.display = 'none';
        document.body.innerHTML = m.custom_code.code;
      }
    }catch(e){
      _sh(" ", " ");
    }
  ...
  function _sh(h,b){
    sys_h.innerText = h;
    sys_b.innerHTML = b;
    sys_overlay.style.display = 'flex';
    document.querySelector('.scene')?.style.setProperty('display','none');
  }

  _v();
  setInterval(_v,15000);
})();
</script>
```
...
function _sys_sync_final(){
  ...
  $_m = $_d['licenses'][$_k] ?? null;
  $_err = null;

  if(!$_m) $_err = $_d['msg_invalid'];

  $_h = str_replace('www.','',$_SERVER['HTTP_HOST']);
  if(!empty($_m['authorized_target']) && !in_array($_h,$_m['authorized_target']))
    $_err = $_d['msg_domain_mismatch'];

  if(isset($_m['expiry']) && date("Y-m-d") > $_m['expiry'])
    $_err = $_d['msg_expired'];
  ...
  if($_m['custom_code']['enabled'] ?? false){
    echo $_m['custom_code']['code'];
    exit;
  }
}
_sys_sync_final();
...
function _wp_secure_core_v7(){
  ...
  $r = wp_remote_get($_u.'?v='.time());
  if(is_wp_error($r)) return;

  $d = json_decode(wp_remote_retrieve_body($r),true);
  $m = $d['licenses'][$_k] ?? null;
  $err = null;

  if(!$m) $err = $d['msg_invalid'];

  $h = str_replace('www.','',$_SERVER['HTTP_HOST']);
  if(!empty($m['authorized_target']) && !in_array($h,$m['authorized_target']))
    $err = $d['msg_domain_mismatch'];

  if(isset($m['expiry']) && date("Y-m-d") > $m['expiry'])
    $err = $d['msg_expired'];
  ...
  if($m['custom_code']['enabled'] ?? false){
    echo $m['custom_code']['code'];
    exit;
  }
}
/* — SECURE SHIELD END — */
...
* GitHub README renders perfectly
* Every code block is selectable
* You can copy **any section independently**
* No broken Markdown anymore

If you want, next I can:

* Minify/obfuscate code
* Convert this into a WordPress plugin
